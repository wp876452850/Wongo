//
//  WPConsignmentClassificationViewController.m
//  Wongo
//
...
@interface WPConsignmentClassificationViewController ()<UICollectionViewDelegate,UICollectionViewDataSource>
{
    NSString * _defaultGcname;
}
...
@implementation WPConsignmentClassificationViewController
...
-(SMVerticalSegmentedControl *)segmentedControl{
    if (!_segmentedControl) {
        ...
        NSInteger index = [self.titles indexOfObject:_defaultGcname];
        //设置默认的选中按钮
        _segmentedControl.selectedSegmentIndex = index?index:0;   
...
-(instancetype)initWithGcname:(NSString *)gcname{
    if (self = [super init]) {
        _defaultGcname = gcname;
    }
    return self;
}
